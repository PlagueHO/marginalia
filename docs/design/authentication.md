...
> **Note:** Entra ID multi-user mode requires Azure AD tenant admin consent and the `Application.ReadWrite.All` Microsoft Graph permission to create app registrations during provisioning.
...
1. Writes the resulting `AZURE_AD_API_CLIENT_ID` and `AZURE_AD_SPA_CLIENT_ID` values back to the `azd` environment.
